...
def main(pop, iter):
    ...
    pos = lot_pool   # positions
    vel = np.zeros((npop, dim))  # velocity
    #objs = [cal_obj(x) for x in pos]  # objectives
    objs = [fc.cal_obj(pop[i]) for i in range(npop)]  # objectives
    ...
    for t in range(iter):
        ...
        for i in range(npop):
            ...
            mid_pop = np.concatenate((mid_pop, pos[i][np.newaxis,:]), axis=0)
            mid_pop = np.concatenate((mid_pop, fac_pool[i][np.newaxis,:]), axis=0)
            mid_objs = fc.cal_obj(mid_pop)
            objs[i] = mid_objs
            ...
            if np.random.random() < pm:
                ...
                mid_pop = np.zeros((0,len(pop[0][0])))
                mid_pop = np.concatenate((mid_pop, new_pos[np.newaxis,:]), axis=0)
                mid_pop = np.concatenate((mid_pop, fac_pool[i][np.newaxis,:]), axis=0)
                new_obj = fc.cal_obj(mid_pop)
    ...
    return rep, rep_obj,final_pop

#pop = fc.creation_pop(76, 6, 5)
#rep,rep_obj = main(pop, 4)
