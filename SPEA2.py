...
def main(pop, iter):
    ...
    objs = np.array([fc.cal_obj(pop[i]) for i in range(npop)])  # the objectives of population
    ...
    arch_semi = np.random.uniform(lb, ub, (narch, dim))  # archive
    arch_pop = []
    for i in range(len(pop)):
        mid_pop = np.zeros((0,len(pop[0][0])))
        mid_pop = np.concatenate((mid_pop, arch_semi[i][np.newaxis,:]), axis=0)
        mid_pop = np.concatenate((mid_pop, fac_pool[i][np.newaxis,:]), axis=0)
        arch_pop.append(mid_pop)
    arch_objs = np.array([fc.cal_obj(arch_pop[i]) for i in range(len(arch_pop))])  # the objectives of archive
    #arch_objs = fc.min_max_normalize(arch_objs)
    ...
    while True:
        ...
        all_pop = np.concatenate((lot_pool, arch_semi), axis=0)
        ...
        if len(index) <= narch:
            rank = np.argsort(F)
            arch_semi = all_pop[rank[: narch]]
            arch_objs = all_objs[rank[: narch]]
            arch_F = F[rank[: narch]]
        else:
            arch_semi = all_pop[index]
        ...
        mating_pool = selection(arch_semi, arch_F, pc)
        offspring1 = crossover(mating_pool, lb, ub, eta_c)
        mutants = selection(arch_semi, arch_F, 1 - pc)
        offspring2 = mutation(mutants, lb, ub, eta_m)
        lot_pool = np.concatenate((offspring1, offspring2), axis=0)
        iteration_pop = []
        for i in range(len(pop)):
            mid_pop = np.zeros((0,len(pop[0][0])))
            mid_pop = np.concatenate((mid_pop, lot_pool[i][np.newaxis,:]), axis=0)
            mid_pop = np.concatenate((mid_pop, fac_pool[i][np.newaxis,:]), axis=0)
            iteration_pop.append(mid_pop)
        objs = np.array([fc.cal_obj(iteration_pop[i]) for i in range(npop)])
        #objs = fc.min_max_normalize(objs)
    final_pop = []
    for i in range(len(pop)):
        mid_pop = np.zeros((0,len(pop[0][0])))
    ...
    return arch_objs,arch_F,final_pop


# pop = fc.creation_pop(76, 10, 5)
# arch_objs,arch_F,final_pop = main(pop, 10)
# # # Step 3. Sort the results
# final = final_pop[np.where(arch_F <= 1)]
